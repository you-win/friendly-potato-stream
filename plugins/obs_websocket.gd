...
func _ready() -> void:
	...
	if obs_client.connect_to_url(URL_PATH) != OK:
		AppManager.console_log("Could not connect to OBS websocket")
...
func _on_connection_closed(_was_clean_close: bool) -> void:
	AppManager.console_log("OBS connection closed")

func _on_connection_error() -> void:
	AppManager.console_log("OBS connection error")

func _on_connection_established(_protocol: String) -> void:
	AppManager.console_log("OBS connection established")
...
func _on_data_received() -> void:
	...
	if typeof(json_response) != TYPE_DICTIONARY:
		AppManager.console_log("Invalid json_response: %s" % json_response)
		return
	...
	AppManager.console_log(message)

func _on_server_close_request(_code: int, _reason: String) -> void:
	AppManager.console_log("OBS close request received")
...
func _get_auth_required() -> void:
	AppManager.console_log("Sending GetAuthRequired message")
...
func _authenticate(auth: String) -> void:
	AppManager.console_log("Sending auth response")
