...
func is_larger_than_or_equal_to(n):
	n = type_check(n)
	calculate(n)
	if is_equal_to(n):
		return true
...
func is_less_than_or_equal_to(n):
	n = type_check(n)
	calculate(n)
	if is_equal_to(n):
		return true
...
func to_aa(no_decimals_on_small_values=false):
	...
	if no_decimals_on_small_values and target == 0:
		result = split[0]
	else:
		if split[0].length() == 3:
